"""
Market data module - Gate.io API integration with rate limiting and caching
(Gate.io is prioritized for China mainland users)
"""
import requests
import time
import threading
...
class MarketDataFetcher:
    ...
    def get_current_prices(self, coins: List[str]) -> Dict[str, Dict]:
        """Get current prices from Gate.io (with stale cache and simulated fallback)"""
        ...
        prices = self._get_prices_from_gateio(coins)

        # If Gate.io failed, try stale cache
        if not prices:
            ...
            print("[WARN] Gate.io failed, using simulated prices")
    ...
    def get_historical_prices(self, coin: str, days: int = 7) -> List[Dict]:
        """Get historical daily prices from Gate.io (with caching)"""
        ...
        # Use Gate.io daily K-lines: map close -> price, keep volume
        kline = self._get_kline_from_gateio(coin, '1d', days)
        prices = [
            {'timestamp': k['timestamp'], 'price': k['close'], 'volume': k['volume']}
            for k in kline
        ] if kline else []
